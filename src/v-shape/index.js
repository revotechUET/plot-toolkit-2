...
function registerEvents(_pixiObj) {
    ...
    const handleMouseDown = (evt) => {
        ...
        this.onmousedown && this.onmousedown(currentTarget, localPos, globalPos, evt);
    ...
    const handleMouseUp = (evt) => {
        let currentTarget = evt.currentTarget;
        let globalPos = evt.data.global;
        this.onmouseup && this.onmouseup(currentTarget, currentTarget.toLocal(globalPos), globalPos, evt);
        if (this.draggable) this.dragEnd(evt.data, currentTarget);
    }
    const handleMouseMove = (evt) => {
        let currentTarget = evt.currentTarget;
        let globalPos = evt.data.global;
        this.onmousemove &&  this.onmousemove(currentTarget, currentTarget.toLocal(globalPos), globalPos, evt);
        if (this.draggable) this.dragMove(evt.data, currentTarget);
    }
    pixiObj.on('mouseover', handleMouseOver)
...
function dragEnd(evtData, target) {
    ...
    this.$parent.getPixiObj().addChildAt(this.pixiObj, this.draggingData.childIdx);
    this.makeScene();
...
function normalizePos(pos) {
    ...
    if (typeof this.dragLimits === "function") {
        ...
        bound = {
            x: this.$parent.posX, y: this.$parent.posY,
        ...
        bound = {
            x: this.dragLimits.posX || 0, y: this.dragLimits.posY || 0,
...
let component = {
    ...
    computed: {
        watchedKeys: function () {
            return ['hasMouseOver', ...(Object.keys(this.$props))].filter(v => v !== 'dragLimits');
        }
    },
    methods: {
        createPixiObj, getMaskObj, registerEvents, draw, dragStart, dragEnd, dragMove, normalizePos
    }
}
